...
func _on_bench_interaction_area_interacted(being_interacted_with: bool) -> void:
	...
	if not being_interacted_with:
		...
		player.animation_controller.set_animation("sitting", true)
		player.animation_controller.set_animation("standing", false)
	else:
		player_state.set_player_state(PlayerStateComponent.PlayerState.IDLE)
		player.animation_controller.set_animation("sitting", false)
		player.animation_controller.set_animation("standing", true)
